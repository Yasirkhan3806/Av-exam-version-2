...
const avgProgress = totalProgress / overallProgress.length;
    ...
    const totalPercentage = overallPercentage.reduce((acc, curr) => acc + curr, 0);
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-blue-50 rounded-lg p-4 text-center transition-all hover:shadow-md hover:bg-blue-100">
          <div className="text-2xl font-bold text-blue-700">{progressData.activeCourses}</div>
          <div className="text-xs font-medium text-blue-600 mt-1">Active Courses</div>
        </div>
        
        {/* Avg Progress */}
        <div className="bg-green-50 rounded-lg p-4 text-center transition-all hover:shadow-md hover:bg-green-100">
          <div className="text-2xl font-bold text-green-700">{progressData.avgProgress}%</div>
          <div className="text-xs font-medium text-green-600 mt-1">Avg Progress</div>
        </div>
        
        
        {/* Avg Grade */}
        <div className="bg-yellow-50 rounded-lg p-4 text-center transition-all hover:shadow-md hover:bg-yellow-100">
          <div className="text-2xl font-bold text-yellow-700">{progressData.avgGrade}</div>
          <div className="text-xs font-medium text-yellow-600 mt-1">Avg Grade</div>
        </div>
      </div>
    </div>
  );
